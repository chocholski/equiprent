...
namespace Equiprent.Web.Controllers
{
    ...
    public class BaseApiController : ControllerBase
    {
        ...
        protected int? CurrentUserId => int.TryParse(User.FindFirst(y => y.Type == ClaimTypes.NameIdentifier)?.Value, out int userId) ? userId : null;
        ...
        protected UserRoleEnum? UserRole
        {
            get
            {
                ...
                return userRoleClaim is not null && Enum.TryParse(userRoleClaim.Value, out UserRoleEnum userRole) 
                    ? userRole 
                    : null;
            }
        }
        ...
        protected ActionResult GetActionResult(CommandResult result, string? message = null)
        {
            return string.IsNullOrEmpty(message)
                ? new JsonResult(result.ToString().Split('_')[1])
