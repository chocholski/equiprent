...
namespace Equiprent.Web.Authorization
{
    public class PermissionPolicyProvider : IAuthorizationPolicyProvider
    {
        ...
        public Task<AuthorizationPolicy?> GetPolicyAsync(string policyName)
        {
            ...
            {
                var permissionsIds = new int[0];

                if (policyName.Substring(POLICY_PREFIX.Length).Equals(typeof(int[]).FullName))
                {
                    permissionsIds = new int[] { (int)UserPermissionEnum.ForAllLoggedIn };
                }
                else
                {
                    permissionsIds = Array.ConvertAll(policyName.Substring(POLICY_PREFIX.Length).Split(","), 
